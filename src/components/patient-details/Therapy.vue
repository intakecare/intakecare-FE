...
/**
 * @component Therapy
 * @description Component to display a therapy.
 *
 * @param {Object} therapy - The therapy to display.
 * @param {Boolean} isSelected - If the therapy is selected.
 *
 * @emits {Object} changed - Emits the updated therapy.
 *
 * @example
 * <therapy :therapy="therapy" :is-selected="isSelected" @changed="updateTherapy" />
 */
...
// i18n and dialog initialization
...
<template>
  ...
  >
    ...
    >
      <n-space vertical>
        <n-text :style="computedTitleStyle">{{therapy.drug.split("-")[0]}}</n-text>
        <!-- Base Information (NOT SELECTED) -->
        <span>
          <n-text :style="computedTextStyle + ' margin-right: 5px'"
            >{{ therapy.posology }} -</n-text
          >
          <n-text :style="computedTextStyle" v-if="therapy.state">{{t("therapies.active")}}</n-text>
          <n-text :style="computedTextStyle" v-else>{{t("therapies.suspended")}}</n-text>
          <n-text v-if="therapy.end_date" :style="computedTextStyle">
            - {{ t("therapies.until") }}: {{ new Date(therapy.end_date).toLocaleDateString(t("code")) }}
          </n-text>
        </span>
